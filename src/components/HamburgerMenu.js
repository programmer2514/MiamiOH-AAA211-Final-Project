...
export default function HamburgerMenu({ page, setPage }) {
  ...
  return (
    <div className='w-8 h-7 m-4 flex flex-col justify-around'>
      ...
      <nav className='fixed right-0 top-28 z-30 h-[calc(100vh-10.5rem)] w-[28rem] max-w-full py-4 px-10 flex flex-col flex-wrap translate-x-full transition-transform bg-[#181818] text-white text-2xl pointer-events-none peer-checked/hamburger:translate-x-0 peer-checked/hamburger:pointer-events-auto max-lg:landscape:h-[calc(100vh-7rem)]' aria-hidden={ !isChecked }>
